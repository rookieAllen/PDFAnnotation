...
class PdfManageViewController: UIViewController,
    ...
    override func loadView() {
        ...
        undoButton.frame = CGRect(x: 900, y: 10, width: 30, height: 30)
    ...
    func _touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {     //NSLog("_touchesMoved")
        ...
        let count = self.lastSelection - arraySelections.count
        ...
        for select : PDFSelection in arraySelections {
            ...
            if cnt > self.lastSelection {
                ...
                if (isHighlited) {
                    ...
                    let count = self.undoAnnotation.last ?? 0 + 1
                    self.undoAnnotation.removeLast()
                    self.undoAnnotation.append(count)
                ...
                if (isUnderline) {
                    ...
                    //追加されたAnnotation数に1を加算する
                    let count = self.undoAnnotation.last! + 1
                    self.undoAnnotation.removeLast()
                    self.undoAnnotation.append(count)
        ...
        self.lastSelection = arraySelections.count
    }
