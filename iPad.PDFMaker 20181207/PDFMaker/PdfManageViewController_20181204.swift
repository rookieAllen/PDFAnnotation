...
penTouchDelegate{
    ...
    func onTapUndoButton(sender:UIButton) {
        ...
        if (self.pdfPageAnnotationDict[nowPage]!.count > 0) {
            ...
            for _ in 1 ... count {
                //touchBeganからtouchEndまでに追加された複数のAnnotationをすべて削除する
    ...
    func _touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for select : PDFSelection in arraySelections {
            //ハイライトする
            if (isHighlited) {
                let annotation : PDFAnnotation = PDFAnnotation(bounds: select.bounds(for: (targetPdfView?.currentPage)!),
            ...
            if (isUnderline) {
                let annotation : PDFAnnotation = PDFAnnotation(bounds: select.bounds(for: (targetPdfView?.currentPage)!),
                                                               forType: PDFAnnotationSubtype.underline, withProperties: nil)
    ...
    func _touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {     //NSLog("_touchesMoved")
        ...
        var reversedSelections : [PDFSelection] = arraySelections.reversed()
        
        print("===========================")
        print(arraySelections)
        print(reversedSelections)
        
        //找到第一块不同的bounds
        var count = -1
        let minCount = arraySelections.count > self.lastSelection.count ? self.lastSelection.count : arraySelections.count
        print("arraySelections = " ,arraySelections.count)
        print("lastSelection = " ,self.lastSelection.count)
        print("minCount = " ,minCount)

        for i in 0 ..< minCount {
            print(self.lastSelection[i] ,((arraySelections[i] as PDFSelection).bounds(for: (targetPdfView?.currentPage)!)))
        }
        
        for i in 0 ..< minCount {
            //如果上一次选中的bounds和这一次选中的bounds相同，则将i赋值给count
            if self.lastSelection[i] == (arraySelections[i] as PDFSelection).bounds(for: (targetPdfView?.currentPage)!) {
                count = i
                print("count1 = ",count)
            } else {
                //如果上一次选中的bounds和这一次选中的bounds不相同，则跳出循环，此时的count是最后一块相同bounds的位置
                break
            }
        }
        
        //从下往上画的时候保存的数组是倒过来存的，所以count为-1
        //此时要将数组倒序来重新进行判断
        if count == -1 {
            //将数组倒序
            //arraySelections = arraySelections.reversed()
            print("-----------------------------------------")
            for i in 0 ..< minCount {
                print(self.lastSelection[i] ,((reversedSelections[i] as PDFSelection).bounds(for: (targetPdfView?.currentPage)!)))
            }
            for i in 0 ..< minCount {
                //如果上一次选中的bounds和这一次选中的bounds相同，则将i赋值给count
                if self.lastSelection[i] == (reversedSelections[i] as PDFSelection).bounds(for: (targetPdfView?.currentPage)!) {
                    count = i
                    print("count2 = ",count)
                } else {
                    //如果上一次选中的bounds和这一次选中的bounds不相同，则跳出循环，此时的count是最后一块相同bounds的位置
        ...
        //将上一次选中的bounds和这一次未选中的bounds从画面上删掉
        for i in count + 1 ..< self.lastSelection.count {
            let lastAnnotation = targetPdfView!.currentPage?.annotations.last
            
            targetPdfView?.currentPage?.removeAnnotation(lastAnnotation!)
            
            self.lastSelection.removeLast()
        ...
        print("<--------------------------------------------------------------------------")
        //将新选中的bounds添加到画面上
        for i in count + 1 ..< arraySelections.count {
            let select: PDFSelection = arraySelections[i] as! PDFSelection
            print(select.bounds(for: (targetPdfView?.currentPage)!))
            //ハイライトする
            if (isHighlited) {
                let annotation : PDFAnnotation = PDFAnnotation(bounds: select.bounds(for: (targetPdfView?.currentPage)!),
                                                               forType: PDFAnnotationSubtype.highlight, withProperties: nil)
                ...
                self.undoAnnotation.removeLast()
                self.undoAnnotation.append(cnt)
                self.lastSelection.append(select.bounds(for: (targetPdfView?.currentPage)!))
